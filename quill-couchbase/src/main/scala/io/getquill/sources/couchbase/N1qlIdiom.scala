...
object N1qlIdiom {
  ...
  implicit def astShow(implicit strategy: NamingStrategy, queryShow: Show[Query]): Show[Ast] =
    Show[Ast] {
      ...
      case a @ (
        ...
        fail(s"Invalid N1ql: '$a'")
    }

  implicit def queryShow(implicit strategy: NamingStrategy): Show[Query] = Show[Query] {
    case q => N1qlQuery(q).show
  }

  implicit def N1qlQueryShow(implicit strategy: NamingStrategy): Show[N1qlQuery] = Show[N1qlQuery] {

    case N1qlQuery(entity, filter, orderBy, limit, select, distinct) =>

      val distinctShow = if (distinct) " DISTINCT" else ""

      val withSelect =
        select match {
          case Nil if distinct => fail(s"N1ql only supports DISTINCT with a selection list.'")
          case Nil             => "SELECT *"
  ...
  implicit def operationShow(implicit strategy: NamingStrategy): Show[Operation] = Show[Operation] {
    ...
    case e: UnaryOperation                                  => fail(s"N1ql doesn't support unary operations. Found: '$e'")
    case e: FunctionApply                                   => fail(s"N1ql doesn't support functions. Found: '$e'")
  }
  ...
  implicit val aggregationOperatorShow: Show[AggregationOperator] = Show[AggregationOperator] {
    ...
    case o                          => fail(s"N1ql doesn't support '$o' aggregations")
  }
  ...
  implicit val binaryOperatorShow: Show[BinaryOperator] = Show[BinaryOperator] {
    ...
    case other                  => fail(s"N1ql doesn't support the '$other' operator.")
  }
  ...
  implicit def valueShow(implicit strategy: NamingStrategy): Show[Value] = Show[Value] {
    ...
    case NullValue           => fail("N1ql doesn't support null values.")
  }
  ...
  implicit def entityShow(implicit strategy: NamingStrategy): Show[Entity] = Show[Entity] {
    case e if e.generated.isDefined => fail(s"N1ql doesn't support returning generated during insertion")
