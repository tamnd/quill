...
class N1qlQuerySpec extends Spec {
  ...
  "distinct query" in {
    ...
    N1qlQuery(q.ast).show mustEqual
  ...
  "invalid n1ql" - {
    "flatMap not supported" in {
      ...
      trap(N1qlQuery(q.ast)).getMessage mustEqual "N1ql doesn't support flatMap."
    }
    "groupBy not supported" in {
      val q = quote {
        qr1.groupBy(t => t.i)
      }
      trap(N1qlQuery(q.ast)).getMessage mustEqual "N1ql doesn't support groupBy."
    }
    "union not supported" in {
      val q = quote {
        qr1.filter(_.i == 0).union(qr1.filter(_.i == 1))
      }
      trap(N1qlQuery(q.ast)).getMessage mustEqual "N1ql doesn't support union/unionAll."
    }
    ...
    "join not supported" in {
      ...
      trap(N1qlQuery(q.ast)).getMessage mustEqual "N1ql doesn't support join."
    }
    "leftJoin not supported" in {
      val q = quote {
        qr1.leftJoin(qr2).on((a, b) => a.i == b.i)
      }
      trap(N1qlQuery(q.ast)).getMessage mustEqual "N1ql doesn't support leftJoin."
    }
    "rightJoin not supported" in {
      val q = quote {
        qr1.rightJoin(qr2).on((a, b) => a.i == b.i)
      }
      trap(N1qlQuery(q.ast)).getMessage mustEqual "N1ql doesn't support rightJoin."
    }
    "fullJoin not supported" in {
      val q = quote {
        qr1.fullJoin(qr2).on((a, b) => a.i == b.i)
      }
      trap(N1qlQuery(q.ast)).getMessage mustEqual "N1ql doesn't support fullJoin."
    }
